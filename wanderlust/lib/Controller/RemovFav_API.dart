...
class RemoveFavAPI {
  RemoveFavAPI(this.context);
  BuildContext context;
  Dio dio = Dio();
  CancelToken canseltoken = CancelToken();
  String myurl = "${apis.Server_Url}""${apis.RemoveFavourite}";
  RemoveFav(int trip_id) async {
    ...
    try {
      ...
      if(response.statusCode == 200){
        await ShowAllFavafter(context).showFavafter(false);
        Provider.of<Trips_Provider>(context,listen: false).setIsfav(false);
